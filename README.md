...
## Сложные типы
Давайте расширим модель пользователя и добавим еще одну:
```csharp
public class User 
{
  public int Id { get; set; }
...
```
Теперь каждый пользователь может иметь автомобили. Почему Cars в модели User имеет тип **IEnumerable**, а не **IQueryable**? Это сделано только для удобства, ко всем IEnumerable коллекциях перед построением фильтра применяется метод **AsQueryable**.
